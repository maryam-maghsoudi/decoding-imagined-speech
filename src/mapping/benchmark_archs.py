...
==========================
Head-to-head comparison of 6 imagined→listened MEG mapping architectures
under identical repeated random train/test splits.

Architectures (all matched to ~50-60K parameters):
  1. LinearLag    — ridge regression with acausal lagged features (baseline)
  2. ShallowMLP   — pointwise-in-time MLP (minimal nonlinearity, no temporal context)
  3. CNN1D        — dilated depthwise-separable 1-D conv, no skip connections
  4. UNet1D       — encoder-decoder 1-D conv with skip connections
  5. RNN          — bidirectional GRU
  6. TCN          — temporal convolutional network (dilated causal conv + residual)

Split strategy (per subject × condition):
  - 8 sessions → train,  2 sessions → test
  - 5 folds with different random seeds
  - 10% of training trials held out for early stopping validation

Training modes for neural models:
  - "full"    : one sample = one full trial  (C, T)
  - "windowed": trials are chunked into fixed-length windows (C, WIN_T)
                → more samples, better stochastic gradient estimates
  LinearLag always uses full trials (lag construction is global over T).

Evaluation (always on full test trials, for direct comparability):
  - MSE
  - Mean Pearson r across channels
  - 4-class correlation-based classification accuracy

Outputs (saved to BENCHMARK_OUT_DIR):
  fold_{k}_results.json          — per-fold metrics, all models × modes
  summary_metrics.json           — mean ± std across folds
  comparison_barplot.png         — grouped bar chart (r, MSE, accuracy)
  per_fold_lineplots.png         — per-fold line plots per metric
"""
...
ALL_MODES    = ["full", "windowed"]    # training modes for neural models
...
class ShallowMLP(nn.Module):
    """
    Pointwise-in-time MLP applied independently at each time step.
    Tests whether cross-channel nonlinear mixing alone helps over linear ridge.
    No temporal context whatsoever.
    """
...
class CNN1D(nn.Module):
    """
    Dilated depthwise-separable 1-D CNN without skip connections.
    Receptive field: ~400ms at 100 Hz via dilations 1, 2, 4, 8.
    """
    def __init__(self, C: int, hidden: int = TARGET_HIDDEN, dropout: float = DROPOUT):
        super().__init__()
...
class UNet1D(nn.Module):
    """
    Encoder-decoder 1-D CNN with skip connections.
    Downsamples ×2 twice; decoder reconstructs full T via transposed convs.
    """
...
def _cat_skip(x: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
    """Trim x to match skip's time dim, then concatenate along channel axis."""
...
class RNN(nn.Module):
    """
    Bidirectional GRU with input/output linear projections.
    Processes full sequence; output at each time step → (C, T).
    """
...
class TCNBlock(nn.Module):
    """Dilated causal conv + residual (WaveNet-style)."""
...
def pearson_r_loss(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Mean (1 - Pearson r) across channels and batch. pred/target: (B, C, T)."""
...
def train_neural(
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        for xb, yb in tr_dl:
            ...
            xb = xb + 0.02 * torch.randn_like(xb)   # mild Gaussian noise augmentation
            opt.zero_grad()
...
def pearsonr_mean(Y_pred: np.ndarray, Y_true: np.ndarray) -> Tuple[float, float]:
    """
    (N, C, T) arrays → (mean_r, median_r) computed by concatenating across
    trials then correlating per channel.
    """
    N, C, T = Y_pred.shape
    rs = []
    for c in range(C):
...
def corr_clf(
    ...
    Y_lis_pool:  np.ndarray,   # (N_pool, C, T)  — listened targets
    test_labels: np.ndarray,   # (N_test,)
    pool_labels: np.ndarray,   # (N_pool,)
) -> Tuple[float, np.ndarray]:
    """
    For each predicted trial: compute mean-channel Pearson r with every pool
    target, average r per class, argmax → predicted class.
    Returns (accuracy, confusion_matrix).
    """
    eps = 1e-8

    def znorm(x: np.ndarray) -> np.ndarray:   # (N, C, T)
...
def run_fold(
    ...
    # ---- full-trial arrays (always needed) ----
    ...
    # ---- windowed arrays (neural "windowed" mode) ----
    ...
    # ------------------------------------------------------------------
    #  LinearLag  (always full trial, no neural training)
    # ------------------------------------------------------------------
    print("\n  [LinearLag] fitting ridge...")
    t0  = time.time()
    lb  = ms_to_samples(LAG_BEFORE_MS)
    la  = ms_to_samples(LAG_AFTER_MS)
    # use tr + val (no early stopping for ridge)
    ...
    for mode in ALL_MODES:
        ...
        for model_name, model in make_models(C).items():
            ...
            Yp      = predict_neural(trained, X_te)   # always evaluate on full trials
...
def plot_results(summary: Dict, model_keys: List[str]) -> None:
    metrics = [
        ...
        ("clf_acc_mean", "clf_acc_std", "4-class accuracy ↑",      "per_fold_clf_acc"),
    ]
    ...
    # ---- full vs windowed difference plot (per neural model) ----
...
def main() -> None:
    ...
    # ---- model keys (in presentation order) ----
